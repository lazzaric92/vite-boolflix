...
<template>
    <div v-if="isLoading === false" class="col-12 h-100 mb-4">
        <div class="d-flex h-100">
            ...
            <div class="item-info h-100 d-flex flex-column justify-content-center ps-4 pe-2">
                ...
                <p v-if="itemInfo.tagline != undefined" class="fst-italic ps-2">{{ itemInfo.tagline }}</p>
                <p v-if="itemInfo.overview != undefined" class="mb-3">{{ itemInfo.overview }}</p>

                <!-- # movie info -->
                <p v-if="this.$route.params.media === 'movie'" class="text-secondary">
                    <!-- year -->
                    <span v-if="itemInfo.release_date != undefined" class="me-2">{{ getYear(itemInfo.release_date) }}</span>
                    <span v-if="itemInfo.release_date != undefined" class="me-2 dot"></span>
                    <!-- runtime -->
                    <span v-if="itemInfo.runtime != undefined">{{ itemInfo.runtime }} min</span>
                </p>

                <!-- # tv info -->
                <p v-else class="text-secondary">
                    <!-- year -->
                    <span v-if="itemInfo.first_air_date != undefined" class="me-2">{{ getYear(itemInfo.first_air_date) }}</span>
                    <span v-if="itemInfo.first_air_date != undefined" class="me-2">-</span>
                    <!-- status -->
                    ...
                    <!-- number of seasons -->
                    <span v-if="itemInfo.number_of_seasons != undefined" class="me-2">{{ itemInfo.number_of_seasons }} {{ (itemInfo.number_of_seasons === 1) ? 'stagione' : 'stagioni' }}</span>
                    <span class="me-2 dot"></span>
                    <!-- number of episodes -->
                    <span v-if="itemInfo.number_of_episodes != undefined" class="me-2">{{ itemInfo.number_of_episodes }} episodi</span>
                    <!-- runtime -->
                    <span v-if="itemInfo.episode_run_time.length === 1" class="me-2 dot"></span>
                    <span v-if="itemInfo.episode_run_time.length === 1" class="me-2">{{ itemInfo.episode_run_time[0] }} min</span>
                </p>

                <!-- # genres -->
                <p class="genres" v-if="itemInfo.genres != undefined">
                ...
                <p class="mb-5">
                <!-- # original language -->
                <span v-if="itemInfo.original_language != undefined" class="lang-icon me-3" :class="`lang-icon-${itemInfo.original_language}` "></span>
    ...
    <div v-if="isLoading === false" class="col-12 ps-4 pe-2 add_info">
        ...
        <!-- # networks -->
        <div v-if="itemInfo.networks != undefined" class="networks mb-5">
            <p class="fs-5" v-if="itemInfo.networks.length > 0">Guarda su:</p>
            <span v-for="(network, index) in itemInfo.networks" :title="network.name">
                <span class="network-name text-secondary">{{ network.name }}</span>
                <span v-if="index < itemInfo.networks.length - 1" class="dot text-secondary mx-3"></span>
            </span>
        ...
        <!-- # production -->
        <p v-if="itemInfo.created_by != undefined" class="mb-3">
            <span>Creato da:</span>
            <span v-for="author in itemInfo.created_by" class="ms-3 text-secondary">{{ author.name }}</span>
        </p>
        <p class="mb-3" v-if="itemInfo.production_companies != undefined && itemInfo.production_companies.length > 0">
            <span class="me-3" v-if="itemInfo.production_companies.length > 0">Prodotto da:</span>
        ...
        <p v-if="itemInfo.production_countries != undefined" class="mb-5">
        ...
        <!-- # collection -->
        <div v-if="itemInfo.belongs_to_collection != undefined">
